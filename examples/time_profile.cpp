...
extern "C"
{
    ...
    int main()
    {
        ...
        double forward_pass_time = 0;
        double update_slack_time = 0;
        double update_dual_time = 0;
        double update_linear_cost = 0;
        ...
        // for (int k = 0; k < NTOTAL - NHORIZON - 1; ++k)
        {
            ...
            // 4. Solve MPC problem
            // solver.work->status = 11;
            // solver.work->iter = 1;

            double update_primal_time = 0;
            double update_slack_time = 0;
            double update_dual_time = 0;
            double update_linear_cost_time = 0;

            // clock_gettime(CLOCK_MONOTONIC, &start);

            
            // std::cout<<"<<std::endl;
            tiny_solve(&solver , &update_primal_time , &update_slack_time ,&update_dual_time , &update_linear_cost_time);


            // clock_gettime(CLOCK_MONOTONIC, &end);
            // time1 = (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
            // std::cout << "Time for iter " << k << ": " << time1 << std::endl;
            std::cout << "Time for  update_primal_time" << ": " << update_primal_time << std::endl;
            std::cout << "Time for  update_slack" << ": " << update_slack_time << std::endl;
            std::cout << "Time for  update_dual_time" << ": " << update_dual_time << std::endl;
            std::cout << "Time for  update_linear_cost_time" << ": " << update_linear_cost_time << std::endl;
